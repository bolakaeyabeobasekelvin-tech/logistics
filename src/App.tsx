...
export default function App() {
  ...
  // Initialize shipments list from localStorage
  useEffect(() => {
    setShipments(getShipments());
  }, []);

  const handleUpdateShipments = (updatedList: Shipment[]) => {
    setShipments(updatedList);
    saveShipments(updatedList);
  };

  const handleResetShipments = () => {
    if (confirm('Are you sure you want to reset the database to original pre-seeded USA & International shipments? Custom edits will be overwritten.')) {
      setShipments(INITIAL_SHIPMENTS);
      saveShipments(INITIAL_SHIPMENTS);
    }
  };
